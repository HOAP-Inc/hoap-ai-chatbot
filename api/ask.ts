...
function localGuard(text: string): { ok: boolean; reply?: string } {
  ...
  // 許可トピックの簡易ホワイトリスト
  // サービス説明・料金・導入・事例・運用・採用課題など
  const allowHints = [
    'サービス', '料金', 'プラン', '導入', '相談', '事例', '支援',
    'instagram', 'インスタ', '採用', '求人', '広報', '問い合わせ',
    'hoap', 'サポート', '効果', 'フロー', '始め方', '契約',
  ]
  const isAllowedTopic = allowHints.some(k => t.includes(k))
  ...
  if (!isAllowedTopic) {
    return { ok: false, reply: 'サービスの話題に限定してる。料金・導入・事例・運用のどれにする？' }
  }
  if (politics.some(k => t.includes(k))) {
    return { ok: false, reply: '政治や思想は扱わない設定にしてる。サービス関連なら答えられるよ' }
  }
  if (asksPII || hasPII) {
    return { ok: false, reply: '個人情報は聞かない運用にしてる。サービス範囲の質問ならどうぞ' }
  }
  return { ok: true }
}
